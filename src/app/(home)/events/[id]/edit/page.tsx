...
const ActivityTable = (props: activityTableProps) => {
  ...
  const deleteActivity = (activityName: string) => {
    setActivitiesInTable(prevActivities => {
      ...
      const updatedActivities = prevActivities.filter(activity => activity.name !== activityName);
  
      // Check if the activity is in newActivities
      const isInNewActivities = newActivities.some(activity => activity.name === activityName);
  
      if (isInNewActivities) {
        setNewActivities(prevNewActivities => prevNewActivities.filter(activity => activity.name !== activityName));
      } else {
        // If it's not in newActivities, check in activities
        const deletedActivity = prevActivities.find(activity => activity.name === activityName);
    ...
    if (index !== -1) {
      ...
      if (isInNewActivities) {
        ...
        setNewActivities(prevNewActivities => {
          ...
          const newActivityIndex = updatedNewActivities.findIndex(activity => activity.name === activityName);
