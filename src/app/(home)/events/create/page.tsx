...
import { DoorClosed, DoorClosedIcon, DoorOpen, Plus } from 'lucide-react'
...
const createEventSchema = Joi.object({
  ...
  numberOfGuests: Joi.number().min(0).required().messages({
  ...
  return (
    <div className="flex h-full w-full flex-grow flex-col items-start text-light-400">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="w-full">
        ...
        />
        {/* this section cotnains  Amount Due, Deposit, and extra ntoe (textarea)*/}
        <Spacer y={2} />
        <Divider className="bg-light-200" />
        <Section
